...
/** @param {any[]} specs @param {any} [decorativeLibs] @param {string | null} [activePromptStyle] */
export function renderCursorrules(specs, decorativeLibs, activePromptStyle = null) {
  ...
  lines.push("");
  lines.push("## AI Coding Loop (Design-anchor must be visible)");
  lines.push("");
  lines.push("- **Preflight exposure**: when a task touches UI, theme, components, tokens, or page layout, start with a visible line: `Design Anchor 预检：我会先查组件规范与 token，优先复用 @design。`");
  lines.push("- **Before writing UI**: choose existing Spec-declared components and token classes first; mention any missing component/token before inventing one.");
  lines.push("- **During edits**: when rules or audit detect raw HTML, hard-coded colors, arbitrary token-sensitive values, or bypassed `@design` components, fix them inside the code generation flow and say: `Design Anchor 自动治理：已将 <button> / hardcoded color / arbitrary spacing 改为组件或语义 token。`");
  lines.push("- **After edits**: run or summarize the Design Anchor self-check in the assistant's task summary. Do not turn these findings into a Portal backlog.");
  lines.push("- **Final answer must include a `Design Anchor 自检` line whenever UI code changed**, covering reused components, token compliance, auto-fixes, unresolved confirmations, and sync/audit status.");
  lines.push("- **Passed summary**: `Design Anchor 自检：复用了 N 个 @design 组件，未发现硬编码颜色，规则已同步。`");
  lines.push("- **Auto-fixed summary**: `Design Anchor 自检：检测到 N 处绕过 @design / token 的实现，已改为使用 Button / DataTable / semantic tokens。`");
  lines.push("- **Needs-confirmation summary**: `Design Anchor 自检：有 N 处需要确认：这个原生 table 是否应该替换为 DataTable？`");
  lines.push("- **Never hide Design Anchor behind generic wording** such as “lint passed” or “checks passed”; name it explicitly in the AI response.");
  lines.push("");
  ...
  lines.push("|---|---|---|---|");
  lines.push("| Token / theme | 改 token、调整 token、看看 token、当前 token、修改主题、调整主题、打开主题编辑器、主题编辑器、改品牌色、改圆角、改间距、切暗色、主题设置 | change token, edit tokens, show tokens, token status, edit theme, adjust theme, open theme editor, theme editor, brand color, radius, spacing, dark mode, theme | `npx design-anchor portal tokens` |");
  ...
  lines.push("| Dashboard / health | 右上角仪表盘、健康度、AI 约束状态、审计、自检 | dashboard, health, AI constraints, audit, self-check | `npx design-anchor portal theme` then use the top-right dashboard icon |");
  lines.push("| Docs / help | 文档、使用说明、怎么接入、CLI 命令 | docs, documentation, how to use, setup, CLI commands | `npx design-anchor portal docs` |");
  lines.push("| Style prompt / tokens | 风格 prompt、品牌风格、从 prompt 提取 token、根据描述生成主题 | style prompt, design prompt, brand style, extract tokens, theme from prompt | `npx design-anchor portal tokens` or `npx design-anchor theme <prompt.md>` |");
  lines.push("");
  lines.push("If the request contains open/show/check/list/change/edit/configure/查看/看看/打开/调整/修改/配置 plus token/theme/theme editor/component/spec/dashboard/health/docs terms, open Portal. For a user-provided style/design prompt, extract tokens with `npx design-anchor theme <prompt.md>` and keep the resulting style guidance subordinate to component specs and semantic tokens. Skip Portal only when the user explicitly says not to open Portal or asks for code-only file edits.");
  lines.push("");
  if (activePromptStyle) {
    lines.push("## Active Design Prompt Style (lightweight B2B layer)");
    lines.push("");
    lines.push("Use this user-provided prompt guidance only after the core contracts above are satisfied. It should influence page rhythm, hierarchy, density, surface treatment, and decorative restraint, not component APIs or token values.");
    lines.push("");
    lines.push(activePromptStyle.trim().replace(/^# /, "### "));
    lines.push("");
  }
...
/** @param {any[]} specs @param {any} [decorativeLibs] @param {string | null} [activePromptStyle] */
export function renderAnchorMarkdown(specs, decorativeLibs, activePromptStyle = null) {
  return `# Design-anchor Rules Mirror\n\nIdentical to root \`.cursorrules\` (generated by \`npm run sync:anchor\`); do not edit manually.\n\n${renderCursorrules(specs, decorativeLibs, activePromptStyle)}`;
}
