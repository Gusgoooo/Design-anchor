...
export function renderCursorrules(specs, decorativeLibs) {
  const forbidden = specs.flatMap((s) => [
    ...(s.forbidden ?? []),
    ...Object.values(s.storyAccord ?? {}).flatMap((frag) =>
  ...
  const lines = [];
  lines.push(
    "# DesignAccord Spec — auto-generated, do not edit manually (after modifying `*.spec.json`, run npm run sync:accord or npm run generate:rules)",
  );
  lines.push("");
  ...
  lines.push("3. **Single source of truth**: component behavior follows `src/accord/schema/components/*.spec.json`; do not infer API from memory.");
  lines.push("");
  ...
  lines.push("- Do not override the following semantics via className (declared in DesignAccord styleLock):");
  specs.forEach((s) => {
    lines.push(`### ${s.componentName} (${s.id})`);
    (s.styleLock?.blacklist ?? []).forEach((b) => {
      lines.push(`- ${b.description} — pattern: \`${patternToString(b)}\``);
    });
    const sh = s.storyAccord && typeof s.storyAccord === "object" ? s.storyAccord : null;
    if (sh) {
...
/** @param {any[]} specs @param {any} [decorativeLibs] */
export function renderAccordMarkdown(specs, decorativeLibs) {
  return `# DesignAccord Rules Mirror\n\nIdentical to root \`.cursorrules\` (generated by \`npm run sync:accord\`); do not edit manually.\n\n${renderCursorrules(specs, decorativeLibs)}`;
}
