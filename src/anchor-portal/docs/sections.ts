...
- A recommended default component library with 60+ governed React + Tailwind components
- An import path for teams that already have their own React + Tailwind .tsx component library to migrate
...
- 一套推荐使用的默认组件库，内置 60+ 个受治理的 React + Tailwind 组件
- 对已有 React + Tailwind 组件库团队提供导入路径：扫描本地 .tsx 组件，再用 spec 固化协议
...
Open the Library tab → pick any component → switch to **Spec.json** tab in the bottom panel → edit → Save. The MCP \`update_schema\` tool exposes the same operation to agents.
...
Library tab → 选任意组件 → 下方面板切到 **Spec.json** → 改 → 保存。MCP \`update_schema\` 工具对 agent 暴露同样的操作。
