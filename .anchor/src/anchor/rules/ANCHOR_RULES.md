...
与根目录 `.cursorrules` 内容一致（由 `npm run sync:anchor` 生成）；请勿手改。

# Design-anchor Spec — 自动生成，请勿手改（修改 `*.spec.json` 后运行 npm run sync:anchor 或 npm run generate:rules）
...
3. **唯一数据源**：组件行为以 `src/anchor/schema/components/*.spec.json` 为准，不凭记忆推断 API。
...
- 禁止通过 className 覆盖下列语义（已在 Design-anchor styleLock 声明）：
### Accordion (accordion)
