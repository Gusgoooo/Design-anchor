...
| 设计系统负责人 | 维护 tokens、specs 和采用规则 | `src/design-tokens`、`src/anchor/schema`、Portal |
| AI agent | 发现组件、读取规则、更新 tokens、自检 | 生成规则文件和 MCP tools |
| 工程负责人 | 让治理可见并进入 CI | Govern 视图、audit profiles、CI 集成 |

## 4. 运行时与仓库布局

在消费项目中，Design-anchor 会创建本地 `.anchor/` 子树和根目录治理文件。

```
your-project/
├── app or src/                         业务应用代码
├── .anchor/                            本地设计系统工作区
│   ├── src/components/base/             React + Tailwind 组件源
│   ├── src/design-tokens/               tokens.json + seed-to-map 管线
...
| 组件源 | `src/components/base/` | 通过 base barrel 导出的 React + Tailwind 受管组件 |
| 组件 specs | `src/anchor/schema/components/*.spec.json` | 用法、forbidden tags、primitives、AI hints 的机器契约 |
| Token 真源 | `src/design-tokens/tokens.json` | seed、暗色覆盖、自定义 seed、派生 token 覆盖 |
...
- **Components**：组件浏览、controls、spec 编辑。
- **Onboarding**：默认 kit、导入现有组件、空白起步。
...
| 命令 | 作用 |
|---|---|
| `anchor start` | 一键 init、install、启动 Portal |
| `anchor init` | scaffold `.anchor/` 和治理文件 |
...
| `anchor upgrade` | 拉取最新模板，同时保留本地产品资产 |
...
- 本地 `.anchor/` 是真实的组件和 Portal 工作区。它应该可版本化、可 review，而不是隐藏在远程服务里。
