...
| 角色               | 典型诉求                            | 产品提供的入口                                                  |
| ---------------- | ------------------------------- | -------------------------------------------------------- |
| 设计师              | 调主题、看组件、维护「组件意图 / AI 语义 / 样式锁定」 | Storybook Portal（DesignToken + 组件预览 + Harness 面板）        |
...
5. 工程师在**项目自有 `src/`** 中写业务；AI 在 Cursor 中受规则与 MCP 约束，优先引用 `@/components/business/*`（路径以实际 monorepo / 别名为准）。

**目录约定（刻意设计）**

- `**.harness/`**：仅组件库与设计资产，**不是**业务应用根目录。
- `**.cursor/`**：仅编辑器与 Agent 侧治理配置（点文件风格，不污染业务源码树命名习惯）。
- **业务代码**：仍放在项目原有的应用 `src/`（或其它约定目录），避免「所有新文件都进组件库」的误用；规则文档中会强调这一点。
